...
- `GATEWAY_CLIENT_MAX_AGE` — таймер принудительного разрыва старых клиентских сессий (секунды).
...
- Telegram иногда кеширует статус профиля прокси; поэтому добавлен таймер принудительного разрыва сессий (`GATEWAY_CLIENT_MAX_AGE`), чтобы доступ отозванного пользователя автоматически обрывался без ручного переподключения.
