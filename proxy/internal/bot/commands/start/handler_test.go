...
//go:generate minimock -g -i github.com/nskondratev/socks5-proxy-server/proxy/internal/bot/commands/start.userStateSetter -o user_state_setter_mock_test.go -n UserStateSetterMock -p start
...
func TestNew(t *testing.T) {
	...
	s := NewUserStateSetterMock(t)
	require.NotNil(t, New(s))
}
...
func TestHandler_Handle(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		assert.Equal(t, "Hello! You can manage proxy server.", c.sent[0])
	})
}
