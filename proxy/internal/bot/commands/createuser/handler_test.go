...
//go:generate minimock -g -i github.com/nskondratev/socks5-proxy-server/proxy/internal/bot/commands/createuser.stateStore -o state_store_mock_test.go -n StateStoreMock -p createuser
...
func TestHandler_Handle(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		s.SetUserStateMock.Set(func(_ context.Context, username string, state store.UserState) error {
			...
			assert.Equal(t, store.StateCreateUserEnterUsername, state.State)
		...
		assert.Equal(t, "Enter username for the new proxy user.", c.sent[0])
	})
}
